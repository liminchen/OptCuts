...
\begin{align}
	\min_{T,U} E_s(T) \quad s.t. \quad E_d(T,U) \leq b_d.
	\label{eq:p1}
\end{align}
\justin{Might be worth mentioning in a sentence what your representation of $T$ is.  Also I moved $b_d$ to the rhs of the inequality.}

Here $E_s$ and $E_d$ respectively measure seam quality and map distortion while $b_d$ is a user specified \emph{upper bound} on acceptable distortion of the generated map. Notice too that this optimization is always feasible as in the limit more cuts will always satisfy any set distortion bound. \danny{If we truly want to make bijectivity a first-class component of our method (I'd suggest we do then we should add the bijectivity constraint to the optimization problem statement above and also the derivation that follows - happy to do this once we decide.}\justin{we enforce local bijectivity regardless, right?  so maybe we just do something generic like $(T,U)\in S$ where $S$ denotes a set of feasible parameterizations.}

In general, distortion measures are smooth albeit \emph{nonconvex}, while seam measures are \emph{nonsmooth} since they can take a discrete jump when mesh edges are unglued.  Hence, we must address both challenges in formulating a suitable %an effective nonsmooth, nonconvex 
optimization method. 
...
%\[ \argmin_{T,U} E_w(T,U) \]
%where $E_w(T,U) = E_s(T) + \lambda E_d(T,U)$ is the total energy combining both seam energy $E_s$ and distortion energy $E_d$ with a balancing factor $\lambda \in \mathbb{R^+}$. Notice that usually $E_d$ is smooth, while $E_s$ is defined to be non-differentiable, which gives us a discrete-continuous problem.

\paragraph{Dual Objective}

Equivalently we can construct the Lagrangian  \justin{why the $w$ in $E_w$?  maybe just call it $L$?}
\begin{align}
...
\begin{align}
	\min_{T,U} \max_{\lambda\geq0} E_w(T,U,\lambda).
	\label{eq:p2}
\end{align}
%
Here $\lambda \in \mathbb{R_+}$ is the Lagrange multiplier for our distortion bound. Notice that the Lagrangian $E_w$ can be interpreted as a multi-objective balancing between distortion and seem quality. In this description, however, $\lambda$ effectively applies a local scaling between the seam and distortion terms that is implied %directly and automatically given 
