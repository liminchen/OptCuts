...
As our second example, we pick the output of the best-performing commercial tool, ZBrush, for two models where OptCuts generated slightly longer seams when starting from Tutte's embedding (see three man and octopus in Table~\ref{tb:comp_commercial}). We now use ZBrush output as initial embedding for OptCuts and run our pipeline to produce polished result. In both cases, OptCuts was able to further shorten the seams (see Figure~\ref{fig:polish_zbrush}).

In all cases OptCuts-polished examples achieve the shortest seam length while maintaining both the distortion bound and bijectivity---highlighting both the utility of polishing and the value of exploiting warm-starts with OptCuts.
...
\caption{UV-map Polishing with OptCuts starting with ZBrush. To demonstrate the value of polishing existing embeddings with OptCuts, we initialize our method with UV map produced by ZBrush. Note that OptCuts shortened seams for both models, while respecting the distortion bound and bijectivity constraints.}
