...
We tested our approach on a benchmark containing a diverse set of 70 surfaces commonly used in geometry processing research.  The meshes from this benchmark  are organic shapes with intricate geometric details composed of XXX to YYY \vova{add numbers} polygons, and are topological disks or spheres \vova{add higher genus if we have it}. In addition, we stress-test the scalability of various methods on a Lucy model that has 48k vertices \vova{faces?} at the highest resolution. 
...
(it is always a triangle soup), but seam penalty is parameterized by $\delta$ parameter which indicates how close the vertices have to be to one another
...
AutoCuts enables the user to control distortion indirectly by adjusting $\lambda$ parameter, but does not guarantee reaching a particular bound. Thus, to make our comparisons fully automatic we we first run AutoCuts on a batch of input surfaces, and then set their output distortions as $b_d$ in our method for each input. Since automatic version of AutoCuts does not naturally support global bijectivity the most direct comparison is to non-bijective version of our method, however, we compare to both variants for completeness.  As demonstrated in Table~\ref{tb:comp_AutoCuts}, for both the two versions of our method, we require less time to generate UV maps with same level of distortion but significantly smaller seams. Figure~\ref{fig:ESLBar_compAutoCuts} provides a more detailed bar chart comparison for the resulting seam lengths across all surfaces. 
