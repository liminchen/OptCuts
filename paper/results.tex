...
Since AutoCuts does not intuitively support generating UV maps with a certain level of distortion, we first run AutoCuts on a batch of input surfaces, and then set their output distortions as $b_d$ in our method for each input. As demonstrated in Table~\ref{tb:comp_AutoCuts}, we require less time to generate UV maps with not larger distortion and significantly smaller seams. Note that we also enforced bijectivity (Figure~\ref{fig:comp_AutoCuts}), which is only supported in AutoCuts with user assistance on patch manipulation.

\begin{table}[!h]
\centering
\caption{Comparisons between AutoCuts and our method on 70 input surfaces with $3721$ vertices per input in average. Two $E_{se}$ are considered to be similar if their ratio lies inside $[0.95, 1.05]$.}
\label{tb:comp_AutoCuts}
\begin{tabular}{@{}cclclccccl@{}}
\toprule
\multirow{2}{*}{method} & \multicolumn{2}{c}{\multirow{2}{*}{$\overline{E}_{SD}$}} & \multicolumn{2}{c}{\multirow{2}{*}{$\overline{E}_{se}$}} & \multicolumn{3}{c}{when our $E_{se}$ is}                                   & \multicolumn{2}{c}{\multirow{2}{*}{$\overline{time}$ (s)}} \\ \cmidrule(lr){6-8}
                        & \multicolumn{2}{c}{}                                  & \multicolumn{2}{c}{}                                  & smaller                 & similar                & larger                  & \multicolumn{2}{c}{}                                  \\ \cmidrule(){1-10}
AutoCuts                & \multicolumn{2}{c}{4.074}                             & \multicolumn{2}{c}{7.579}                             & \multirow{2}{*}{85.5\%} & \multirow{2}{*}{7.2\%} & \multirow{2}{*}{7.2\%} & \multicolumn{2}{c}{500.0}                             \\
ours                    & \multicolumn{2}{c}{4.073}                             & \multicolumn{2}{c}{5.811}                             &                         &                        &                         & \multicolumn{2}{c}{304.0}                             \\ \cmidrule(){1-10}
\end{tabular}
\end{table}
