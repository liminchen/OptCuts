...
\minchen{show several different initial cuts for closed surfaces and high-genus surface to prove that we won't be affected by initial embeddings, we can always find a local optimum w.r.t. both $T$ and $U$} To show that we search for locally optimal UV maps regardless of the given initial embedding, we run our method starting from our standard initializations, triangle soups, and preliminary UV maps produced by other methods or by users. As demonstrated in Figure~\ref{fig:bad_init_still_ends_well}, all the output UV maps are with high quality.
...
\minchen{better quality since EB strategy and Seamster are more global, but we can still improve, this can also benefit user prefered initial maps, and save running time}
