...
\[\hat{f}_e(e^{(k,i-1),j}_{T}) \leftarrow \Big(E^j_s + \lambda^{k+1} \min_{U^{(k,i-1),j}} E_d(T^j,U)\Big) - L(T^{k,i-1},U^{k,i-1},\lambda^{k+1})\]
 We only moves to \justin{didn't parse ``moves to''}\minchen{graph search..} $v^j_T$ with smallest $\hat{f}_e(e^{(k,i-1),j}_{T})$, if this energy decrease is smaller than $\delta^{k,i-1}$.
...
\subsection{Basic Topological Operations}
