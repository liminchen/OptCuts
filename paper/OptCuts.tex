...
\centering
  ...
  \caption{Starting with an initial embedding (left), our OptCuts optimization algorithm iteratively updates UV mesh topology and vertex positions, while introducing minimal-length seams to achieve any user-provided 
  distortion bound, here set to $b_d=4.1$, measured using symmetric Dirichlet energy~\cite{Smith2015Bijective}. We jointly optimize for continuous changes in the embedding and discrete topological changes by propagating simple and local topology splitting and merging operations. Here we report total inner iterations at each rendered frame. }
  \label{fig:teaser}
\end{teaserfigure}
