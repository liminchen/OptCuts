...
\centering
  ...
  \caption{Starting with an initial embedding (left), our method iteratively updates mesh topology, aiming to introduce minimal seams to achieve the target
  distortion bound $b_d=4.1$ (measured using symmetric Dirichlet energy~\cite{Smith2015Bijective}). We jointly optimize for discrete topological changes and continuous changes in the embedding. We achieve the local optima for our constrained optimization problem with simple and local topology updates: splitting an edge near a boundary, splitting an edge on the interior of the mesh, and merging an edge.}
  \label{fig:teaser}
\end{teaserfigure}
