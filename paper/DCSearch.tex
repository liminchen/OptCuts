...
Thus our problem could be restated as to start from a $v^0_T$, search for a $v^i_T$ in $\mathcal{G}_T$ where all edges started from it satisfies $f_e(e^{i,j}_T) \geq 0$ ($e^{i,j}_T \in \mathcal{E}^i_T$).\justin{previous sentence didn't parse for me}
...
As our method is defined to guarantee convergence to a local optimum, we now analyze the convergence rate. First, it is easy to see that $E_w$ monotonically decreases \justin{not sure what this means:} looking at each end of descent steps \minchen{[NOTE] might change to abandon roll back}. Now we look at descent step $i$ and $i+1$, from $E^i_w \geq E^{i+1}_w$ we have
...
\justin{nothing about this discussion seems to involve convergence rate.}